...
%set of ground atomic formulas defined over the $\cal{L}'$ predicates where all parameters are bound, i.e., replaced by a constants from $C$. A state $s$ is a model that assign {\it true} to all ground atomic formulas in $s$, and {\it false} to all ground atomic formulas not in $s$. Thus, a negated formula $\neg\phi$ is {\it true} in a state $s$, noted $s \models \neg \phi$, if only $\phi$ is not in $s$;  $s \models \phi_1 \wedge \phi_2$ if only both $\phi_1$ and $\phi_2$ is {\it true} in $s$; $s \models \phi_1 \vee \phi_2$ if $\phi_1$ or $\phi_2$ is {\it true} in $s$ and $s \models \phi_1 \rightarrow \phi_2$ if  $s \models \phi_1 \vee \neg \phi_2$; $\forall x \phi$ if for all possible values of $x$ in $C$ $s \models \phi$ and $\exists x \phi$ if it exists at least one value of $x$ in $C$ s.t $s \models \phi$.
 ...
 A snap action is an action whose execution is instantaneous, in the sense of classical planning, meaning that they have a duration $\epsilon \ll \delta$ between preconditions checking and applying effects. %% Using Greek letters to denote time
 %
 \begin{definition}[Snap Action] A \emph{snap action} $a$ is a tuple $\bigl(\name(a), \pre(a), \effect(a) \bigr)$, where $\name(a)$ is the name of $a$, the precondition $\pre(a)$ is a first-order formula, % $\phi$,
...
\draw[cyan, line width=.25cm] (2 cm,0.15) -- +(4,0);
...
%(\x,-.7) -- +(1,0);
\draw[>=latex, <->, dashed, teal] (1,-.9) --  +(6,0);
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex, teal] at (8/2,-.65) {$\delta$};

% braces
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (1,.65) {\emph{pre}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (2,.65) {\emph{eff}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (1.5,.3) {start(a)};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (6,.65) {\emph{pre}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (7,.65) {\emph{eff}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (6.5,.3) {end(a)};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (4,.15) {inv(a)};
...
A \emph{method} $m$ is a tuple $\bigl(\name(m), \task(m), \tn(m)\bigr)$, where $\name(m)$ is the name of the method, $\task(m)$ is the task refined by the method, and $\tn(m)$ is the temporal task network decomposing $\task(m)$ into the subtasks $\{t_m \in \cal{I}\}$.
\end{definition}
%
 We can now define the planning domain and problem for Temporal HTN Planning.
 %
 \begin{definition}
 A \emph{planning domain} $\cal{D}$ is a tuple $({\cal{L}}, {\cal{T}}, {\cal{A}}, {\cal{M}})$, where $\cal{L}$ is the first-order logic language, $\cal{T}$ is the set of tasks, ${\cal{A}}$ is a set of actions and $\cal{M}$ is the set of methods. %We note ${\cal{T}}_p$ the set of primitive tasks and ${\cal{T}}_a$ the set of abstract tasks, s.t.  ${\cal{T}} = {\cal{T}}_p \cup {\cal{T}}_a$.
 \end{definition}
...
\begin{definition}[Executable Temporal Sequence of Tasks]
  ...
  (iii) given a tuple $(t_i, e_i, \delta_i) \in \pi$, with $0 \leq i \leq n$, for a state $s_i$ at date $e_i$ we have: 
\begin{itemize}
  \item $t_i$ is achieved by a ground action $a_i$; %% Useless
  \item for the set  $B_{e_i}$ of all the snap actions executing at $e_i$, s.t.:
    \begin{multline*}
 ...
 and for the set $I_{e_i}$ of the invariants holding a $e_i$, s.t.:
    \begin{equation*}
      I_{e_i} = \bigl\{ \tinv(a_j) \; | \; (a_j, e_j, \delta_j) \in \pi \wedge e_j < e_i < e_j + \delta_j\bigr\}
    \end{equation*}

    at $e_i$, a state $s_i$  transitions in a new state $s_{i+1}$, given $B_{e_i}$ and $I_{e_i}$, and that the snap actions in $B_{e_i}$ are \emph{pairwise non-interfering}:   %transition function between two states
      \begin{align*}
