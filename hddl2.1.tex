...
%% Domain def %%%
 ...
 We now define the planning domain and problem for Temporal HTN Planning using the
 %
 \begin{definition}
   A \emph{planning domain} $\cal{D}$ is a tuple $({\cal{L}}, {\cal{T}}, {\cal{I}}, \alpha, {\cal{A}}, {\cal{M}})$, where
   $\cal{L}$ is the first-order logic language,
   $\cal{T}$ is the set of tasks,
   $\cal{I}$ is the set of task identifiers~\footnote{Task identifiers are arbitrary symbols, which serve as place holders for the actual tasks they represent. Identifiers are needed because tasks can occur multiple times within the same task network.},
   $\alpha: {\cal{I}} \rightarrow {\cal{T}}$ is the funciton that maps task identifiers to tasks,
 ...
 A snap action is an action whose execution is instantaneous, in the sense of classical planning, meaning that they have a null duration $\epsilon = 0$ %$\epsilon \ll \delta$
 between preconditions checking and applying effects. %% Using Greek letters to denote time
 %
...
How states change will be formally defined in Section \ref{THTN}.
...
For instance, in the task of serving a dinner, {\it deliver-dinner(?food-style, ?place)} is the compound task consisting in performing first the task of serving the starters, then the main course, etc. In that sense,  {\it deliver-dinner(?food-style, ?place)} can be refined in: {\it serve-starters(?food-style, ?place)},  {\it serve-main-course(?food-style, ?place)}, etc. This mapping between tasks is achieved by the Temporal Task Networks and the methods as defined below. Like primitive tasks, compound tasks have also a start event and an end event, which will be associated to time points in Section \ref{THTN}. % time points  =  dates

\begin{figure}
\begin{tikzpicture}
% draw horizontal line   
\draw[thick, -Triangle] (0,0) -- (8cm,0) node[font=\scriptsize,below left=3pt and -8pt]{time};
...
\draw[cyan, line width=.25cm] (1.05 cm,0.15) -- +(4.9,0);
\draw [yellow, arrows = {-Stealth[length=10pt]}, line width=1mm] (1 cm, 0) -- +(0, 0.75);
\draw [yellow, arrows = {-Stealth[length=10pt]}, line width=1mm] (6 cm, 0) -- +(0, -0.75);
...
%(\x,-.7) -- +(1,0);
\draw[>=latex, <->, dashed, teal] (1,-.75) --  +(5,0);
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex, teal] at (7/2,-.5) {$\delta$};

% braces
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (0.75,.5) {\emph{pre}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (1.25,.5) {\emph{eff}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (1,-.15) {start(a)};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (5.75,.5) {\emph{pre}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (6.2,.5) {\emph{eff}};
\node[font=\scriptsize, text height=1.75ex,text depth=.5ex] at (6,-.15) {end(a)};
...
%       node [black,midway,font=\scriptsize, below=4pt] {$\epsilon$};
\end{tikzpicture}
\caption{Timeline of a durative action \emph{a} application.\label{fig:timeline}}
\end{figure}
...
A temporal task network $w = ({\cal{I}}', {\cal{C}})$ is given by: % is a partially temporal ordered multi-set of tasks $\cal{T}$, where %% Where is the multi-set ?
\begin{itemize}

    \item $\cal{I}' \subseteq \cal{I}$ is a set (possibly empty) set of tasks identifiers,
    ...
    \item $\cal{C}$ is the set of constraints, with ${\cal{C}} = < {\cal{C}}_o, {\cal{C}}_v, {\cal{C}}_d, {\cal{C}}_t) >$:
      ...
      \end{itemize}
\end{itemize}
$w$ implicitely defines a temporal ordered multi-set of tasks ${\cal{T}}' = \{ \alpha(i) \: | \: i \in {\cal{I}}' \}$.
\end{definition}
...
\begin{definition}[Temporal Sequence of Tasks]
  A \emph{temporal sequence of tasks} $\pi$ over a planning domain $\cal{D}$, is a sequence of tuples $\langle (t_0, e_0, \delta_0), \ldots,$ $(t_n, e_n, \delta_n) \rangle$ where $t_0, \ldots, t_n$ are the set of ground tasks defined over $\cal{T}$, $e_i \in \mathbb{N}_{\geq 0}$ and $\delta_i \in \mathbb{N}_{\geq 0}$ are natural numbers~\footnote{
    Rational numbers are used in the usual definition from~\cite{}. However, we feel that using rational numbers is a huge mistake, as the consequence is that the problem might be undecidable.}
  to which refer as the starting time event and the duration of the task $t_i$, respectively. For a temporal sequence of tasks $\pi$, we call a sorted sequence $e_0, \ldots, e_n$ of the set of natural numbers $\{e \ | \ (t, e, d) \in \pi\} \cup \{e + d \ | \ (t, e, d) \in \pi\}$ the happening events of the sequence $\pi$. % denoted $htps(\pi)$.  These are dates.
A temporal sequence of tasks  $\pi$ is primitive if and only if every task $t$ s.t. $(t, e, d) \in \pi$ is primitive, i.e. it is be carried out by an action (either snap or durative action).
\end{definition}
...
%% Manque l'exécution par rapport au temps. The execution of a temporal sequence of tasks results in a sequence of states <s_i, e_i>
 ...
 This definition can easily be extended to a temporal task network.
  ...
  \begin{definition}[Executable Temporal Task Network] A temporal task network $w = ({\cal{I}}, \alpha, {\cal{C}}_o, {\cal{C}}_d, {\cal{C}}_t)$ is executable in a state $s_0$ if and only if there exists an executable temporal sequence of tasks $\pi = \langle (\alpha(i_0), e_0, \delta_0), \ldots,$ $(\alpha(i_n), e_n, \delta_n)\rangle$ in $s_0$ where $i_0, \ldots, i_n$ are task identifiers in $\cal{I}$ that matches the following conditions: (i) $\pi$ matches the temporal constraints ${\cal{C}}_o$, (ii) $\pi$ matches the duration constraints ${\cal{C}}_d$, and (iii) the sequence of states $\langle (s_0, e_0, \delta_0), \ldots,$ $(s_n, e_n, \delta_n) \rangle$ resulting from the execution of $\pi$ matches the constraints ${\cal{C}}_t$.
  \end{definition}
  ...
  refines a task $t = \task(m)$ in the subtasks multi-set  $\{\alpha^m(i)   \, | \, i \in {\cal{I}}^m \}$ with
  $\tn(m) = ({\cal{I}}^m, \alpha^m, {\cal{C}}_{o}^{m},{\cal{C}}_{d}^{m}, {\cal{C}}_{t}^{m})$ the temporal task network of $m$.
  
  % solution or plan if forall t in w_i, t verifies the contraints C_i

\begin{definition}[Decomposition]
%  Let $m = \bigl(\name(m),$ $\task(m), tn(m)\bigr)$ be a method with a task network $tn(m) = ({\cal{I}}^m, \alpha^m, {\cal{C}}_{o}^{m},{\cal{C}}_{d}^{m}, {\cal{C}}_{t}^{m})$ and
  
  A task network $w_1= ({\cal{I}}^1, \alpha^1, {\cal{C}}_{o}^{1},{\cal{C}}_{d}^{1}, {\cal{C}}_{t}^{1})$ is decomposed into a new task network
$w_2= ({\cal{I}}^2, \alpha^2, {\cal{C}}_{o}^{2},{\cal{C}}_{d}^{2}, {\cal{C}}_{t}^{2})$ by refinement by a method $m = \bigl(\name(m),$ $\task(m), tn(m)\bigr)$ if and only if exists an $i \in {\cal{I}}^1 \text{ such that } \alpha^1(i) = task(m)$ and
...
\subsection{Temporal Constraint Semantics}
...
%
\begin{multline*}
\langle (s_0, e_0, \delta_0), \ldots, (s_i, e_i, \delta_i), \ldots, (s_n, e_n, \delta_n) \rangle \models \\
...
\end{multline*}
%
A constraint \sometimes~ says that some properties must be verified sometimes in the decomposition of the task network.
%
\begin{multline*}
\langle (s_0, e_0, \delta_0), \ldots, (s_i, e_i, \delta_i), \ldots, (s_n, e_n, \delta_n) \rangle \models  \\
...
%
\begin{multline*}
\langle (s_0, e_0, \delta_0), \ldots, (s_n, e_n, \delta_n) \rangle \models (\atmostonce \ \phi) \\
...
\end{multline*}
%
Finally, we have the three last constraints introduced in PDDL~3.0: \sometimebefore, \sometimeafter~ and \alwayswithin. A constraint \sometimebefore~ (respectively \sometimeafter~) says that if some properties $\phi$ occur at $e_i$ then $\psi$ must be verified sometime before (respectively after) $e_i$ in the sequence of states produced by the decomposition of the task network.
%
\begin{multline*}
\langle (s_0, e_0, \delta_0), \ldots, (s_i, e_i, \delta_i), \ldots, (s_n, e_n, \delta_n) \rangle \models   \\
(\sometimebefore \ \phi \ \psi) \ \text{iff} \; \forall e_i \ \text{if} \ (\at \ e_i \ \phi)\\
\exists e_j : 0 \leq e_j \leq e_i \: \text{such as} \ (\at \ e_j \ \psi)\qquad\qquad\quad\\
\langle (s_0, e_0, \delta_0), \ldots, (s_i, e_i, \delta_i), \ldots, (s_n, e_n, \delta_n) \rangle \models  \qquad \\
(\sometimeafter \ \phi \ \psi) \ \text{iff} \; \forall e_i \ \text{if} \ (\at \ e_i \ \phi)\\
\exists e_j : e_i \leq e_j \leq e_n \: \text{such as} \ (\at \ e_j \ \psi) \qquad\qquad\qquad
\end{multline*}
%
...
The proposed syntax, called HDDL2.1, takes its roots in HDDL~\citep{holler20}. In a general manner, the reader is referred to the Appendix for the full syntax\footnote{The full syntax will be available as appendix in the final paper version.}, including the elements already present in previous versions of PDDL and HDDL.
...
%
% @HDDL 1.0
%% xxx pas de numeros de lignesxxx
...
%
% @HDDL 2.1
HDDL~1.0 only accepts equality constraints or inequality on method parameters in the task network. To increase the expressiveness of the language, we have chosen to add two kinds of constraints: the \emph{classical decomposition constraints} ($\before$, $\after$, $\between$) used in HTN planning introduced first by \citep{erol94} to represent constraints between tasks for non temporal HTN problem and the \emph{temporal decomposition constraints} based on the plan trajectory constraint from PDDL~3.0. The semantics of these constraints have been given in Section~\ref{Sec:Decomposition-Constraints-Semantics}. To allow the specification of these constraints, it is necessary to use \verb+:method-constraints+.
