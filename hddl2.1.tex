...
\begin{definition}[Temporal Sequence of Tasks] % TODO e_i is an event and \delta is a date/time
  ...
  For a temporal sequence of tasks $\pi$, the set of dates $\{e \ | \ (t, e, \delta) \in \pi\} \cup \{e + \delta \ | \ (t, e, \delta) \in \pi\}$  define a sorted sequence $<e_0, \ldots, e_n>$ of \emph{happening events} of $\pi$. % denoted $htps(\pi)$.  These are dates. TODO "ordered sequence" ?
A temporal sequence of tasks  $\pi$ is primitive if and only if every task $t$ s.t. $(t, e, \delta) \in \pi$ is primitive, i.e. it is be carried out by an action (either snap or durative).
\end{definition}
...
%  Let $m = \bigl(\name(m),$ $\task(m), tn(m)\bigr)$ be a method with a task network $tn(m) = ({\cal{I}}^m, \alpha^m, {\cal{C}}_{o}^{m},{\cal{C}}_{d}^{m}, {\cal{C}}_{t}^{m})$ and
  ...
  A task network $w_1= ({\cal{I}}^1, {\cal{C}}_{o}^{1},{\cal{C}}_{d}^{1}, {\cal{C}}_{t}^{1})$ is decomposed into a new task network
$w_2= ({\cal{I}}^2, {\cal{C}}_{o}^{2},{\cal{C}}_{d}^{2}, {\cal{C}}_{t}^{2})$ by refinement by a method $m = \bigl(\name(m),$ $\task(m), tn(m)\bigr)$ if and only if exists an $i \in {\cal{I}}^1 \text{ such that } \alpha(i) = task(m)$ and
...
\end{multline*}
%
Constraint \within~ says that some properties must be verified sometime before the begin of the execution of a specific event. %TODO: action?
%
\begin{multline*}
\langle (s_0, e_0, \delta_0), \ldots, (s_i, e_i, \delta_i), \ldots, (s_n, e_n, \delta_n) \rangle \models   \\
