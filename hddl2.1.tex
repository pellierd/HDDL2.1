...
%% Domain def %%%
 ...
 This mapping between tasks is achieved by a set of decomposition methods, namely the methods (Def. \ref{def:met}) and the the Temporal Task Networks (Def. \ref{def:TNN}).%as defined below in Def.~\ref{def:decomp}.
 ...
 \begin{definition}
   ...
   $\cal{I}$ is the set of task identifiers~\footnote{Task identifiers are arbitrary symbols, which serve as place holders for the actual tasks they represent. Identifiers are needed because tasks can occur multiple times within the same task network, as we will see below.},
...
\end{figure}
 ...
 A snap action is an action whose execution is instantaneous in the sense of classical planning, meaning that it has a null duration % $\epsilon = 0$  -- $\epsilon \ll \delta$
...
States transition will be formally defined in Section \ref{THTN}.  % TODO Check
...
A compound task is identified by a name and defines the way other --possibly ordered-- tasks (either primitive or compound) must be achieved with respect to some constraints in order to refine it.
 Like primitive tasks, compound tasks have also a start event and an end event, which will be associated to time points in Section \ref{THTN}. % time points  =  dates
...
\begin{definition}[Temporal Sequence of Tasks] % TODO e_i is an event and \delta is a date/time
  ...
  to which refer as the starting time event and the duration of the task $t_i$, respectively. For a temporal sequence of tasks $\pi$, we call a sorted sequence $e_0, \ldots, e_n$ of the set of natural numbers $\{e \ | \ (t, e, \delta) \in \pi\} \cup \{e + \delta \ | \ (t, e, \delta) \in \pi\}$ the happening events of the sequence $\pi$. % denoted $htps(\pi)$.  These are dates.
A temporal sequence of tasks  $\pi$ is primitive if and only if every task $t$ s.t. $(t, e, \delta) \in \pi$ is primitive, i.e. it is be carried out by an action (either snap or durative action).
\end{definition}
...
%% Manque l'exécution par rapport au temps. The execution of a temporal sequence of tasks results in a sequence of states <s_i, e_i>
 ...
 This definition can easily be extended to a temporal task network.
  ...
  \begin{definition}[Executable Temporal Task Network] A temporal task network $w = ({\cal{I}}', <\!{\cal{C}}_o, {\cal{C}}_v, {\cal{C}}_d, {\cal{C}}_t\!\!>)$ is executable in a state $s_0$ if and only if there exists an executable temporal sequence of tasks $\pi = \langle (\alpha(i_0), e_0, \delta_0), \ldots,$ $(\alpha(i_n), e_n, \delta_n)\rangle$ in $s_0$ where $i_0, \ldots, i_n$ are task identifiers in $\cal{I}'$ that matches the following conditions: (i) $\pi$ matches the temporal constraints ${\cal{C}}_o$, (ii) $\pi$ matches the duration constraints ${\cal{C}}_d$, and (iii) the sequence of states $\langle (s_0, e_0, \delta_0), \ldots,$ $(s_n, e_n, \delta_n) \rangle$ resulting from the execution of $\pi$ matches the constraints ${\cal{C}}_t$.
  \end{definition}
