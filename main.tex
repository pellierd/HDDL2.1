...
This paper presents a proposed BNF syntax to extend HDDL to deal with time and numeric. The proposed syntax, called HDDL2.1, takes its roots in HDDL~\citep{holler:19}. Two main extensions are proposed:
\begin{enumerate}
    \item Allow the expression of temporal methods by allowing the definition of temporal constraints between the tasks of a method.
    \item  Allow the declaration of logical constraints on method decompositions inspired by the on-plane constraints defined in PDDL3.0.
\end{enumerate}
