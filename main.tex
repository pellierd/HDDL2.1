...
This proposed syntax for HDDL2.1 takes its roots in HDDL~\citep{holler:19}. From that initial definitions, we here describe its proposed extension.
...
<require-def> ::= ~\linebreak~(:requirements <require-key>+)
<require-key> ::= ~\textit{see below}~
<types-def> ::= (:types ~\mbox{<typed-list(<primitive-type>))}~
<predicates-def> ::= ~\linebreak~(:predicates <atomic-formula-skeleton>+)
<function-def> ::= ~\linebreak~(:<function-typed-list (atomic-function-skeleton)>)
<constants-def> ::= ~\linebreak~(:constants <typed-list (constant)>)
...
%
% @HDDL 1.0
\noindent Abstract tasks are defined similarly to actions.
\begin{lstlisting}[firstnumber=last, escapechar=~]
