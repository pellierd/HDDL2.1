...
Durative methods may have preconditions and their preconditions are temporally annotated to give the time at which they are supposed to happen. As durative actions, methods can have three types of annotated preconditions: (1) $(\atstart \ \phi)$ meaning that $\phi$ must be true or happen at the start of the method; (2) $(\atend \ \phi)$ meaning that $\phi$ must be true or happen at the end of the method, and $(\over all \ \phi)$ meaning that $\phi$ must be true for the full duration of the method.


Formally, these three temporal preconditions can be rewritten in terms of decomposition constraints previously introduced. A precondition of type $(\atstart \ \phi)$ (respectively $(\atend \ \phi)$) is equivalent to adding a decomposition constraint $(\atstart \ \phi)$ (respectively $(\atend \ \phi)$), and a precondition of type $(\over all \ \phi)$ is equivalent to adding a decomposition constraint $(\always \ \phi)$. The preconditions of the methods can be seen as syntactic glue to express decomposition constraints of type $\atstart$, $\atend$ and $\always$.
...
To be fully compatible with PDDL 3.0 features, the language HDDL 2.1 needs to include axioms and preferences, besides the associated parsing, and validating tools.
Future works include,
\begin{itemize}
\item To define constraints on solution plans, e.g. put a max time linit to plans
\item To allow  defining delays for ordering constraints with point algebra
\item All other aspects not detailed here, e.g. continuous effects
\end{itemize}
...
\section*{Temporal HTN planning Semantics}
 ...
 The solution of a temporal HTN planning problem is a time-triggered plan that might be obtained from the problem initial task network by a series of method decompositions. Lifted problems are just a compact representation of their ground instances. With ground instances, there is no need to use variable constraints since they are satisfied by the grounding. Therefore, in this section, for the sake of simplicity, we define the semantics of a lifted problem in terms of its ground instances. For details on the grounding process, the reader is referred to \cite{behnke20,ramoul17}.

% Let us start by informally discussing the role of this hierarchical extension over the underlying PDDL language.
% A PDDL problem characterizes the initial state and its evolution overtime as a result of the execution of actions and of its intrinsic dynamics (e.g. with time initial literals).


\begin{definition}[Time-triggered plan]
A {\it time-triggered plan} $\pi$ is a sequence of tuples $\langle (t_0, st_0, d_0), \ldots,$ $(t_n, st_n, d_n) \rangle$ where each $t_i$ is a ground primitive task in $\cal{T}$, $st_i \in \mathbb{Q}_{\geq 0}$ and $d_i \in \mathbb{Q}_{\geq 0}$ are rational numbers respectively giving the start time and the duration of the primitive task $t_i$.
\end{definition}

\newcommand{\states}{\Psi}
\begin{definition}[State trajectory]
    Given a problem $P$, a time-triggered plan induces a state trajectory $\states^P_\pi: \mathbb{R}^{\geq 0} \rightarrow S$.
\end{definition}

The precise characterization of the state trajectory is not in the scope of this paper.
Indeed, observe that for a given time-triggered plan, the state trajectory solely depends on the semantics associated to the action model and state dynamics, both of which are not impacted by this hierarchical extension.
Instead, the hierarchical extension only restricts the set of plans that can be accepted as a solution to the problem.

In essence, this allows us to define the semantics of this hierarchical extension orthogonally to the underlying action model as long as it allows to test whether a given formula $\phi$ holds at a given time $t$, in the state trajectory induced by the plan $\pi$ (i.e. $\states_\pi(t) \models \phi$).


\begin{definition}[Decomposition]
Let be a method \mbox{$m = \bigl(\name(m),$ $\task(m), tn(m)\bigr)$} with a task network $tn(m) = ({\cal{I}}^m, \alpha^m, {\cal{C}}_{o}^{m},{\cal{C}}_{d}^{m}, {\cal{C}}_{t}^{m})$ and a task network to decompose $w_1= ({\cal{I}}^1, \alpha^1, {\cal{C}}_{o}^{1},{\cal{C}}_{d}^{1}, {\cal{C}}_{t}^{1})$. Without loss of generality we assume ${\cal{I}}^m \cap {\cal{I}}^1 = \emptyset$ (which might be achieved by renaming). Then, $m$ decomposes a task identifier $i \in {\cal{I}}^1$ into a task network $w_2= ({\cal{I}}^2, \alpha^2, {\cal{C}}_{o}^{2},{\cal{C}}_{d}^{2}, {\cal{C}}_{t}^{2})$ if and only if $\alpha^1(i) = task(m)$ and
\begin{equation*} \label{eq1}
\begin{aligned}
{\cal{I}}^2 = & ({\cal{I}}^1 - \{i\}) \cup {\cal{I}}^m\\
\alpha^2 = & (\alpha^1 \cup \alpha^m)\\
{\cal{C}}^{2}_{o} = & {\cal{C}}^{1}_{o} \cup {\cal{C}}^{m}_{o} \\
    & \cup \{ \ \tstart(i) = \min_{j \in {\cal{I}}^m} \tstart(j) \}  \\
    & \cup \{ \ \tend(i) = \max_{j \in {\cal{I}}^m} \tend(j) \}  \\
{\cal{C}}^{2}_{d}  = & {\cal{C}}^{1}_{d} \cup {\cal{C}}^{m}_{d} \\
{\cal{C}}^{2}_{t}  = & {\cal{C}}^{1}_{t} \cup {\cal{C}}^{m}_{t} \\
\end{aligned}
\end{equation*}
...
\begin{definition}
    TODO: A task network $w'$ is a decomposition of a task network $w$ if there is a sequence of method application that transform $w$ into $w'$.
\end{definition}

\begin{definition}[Schedule]TODO:
    A time-triggered plan $\pi$ is a valid schedule of a decomposition $w$ if:
    \begin{itemize}
        \item there is a bijection between the actions of the plan and the primitive tasks of the decomposition.
        \item Note that this bijection assigns a numerical value to all timepoints in the decomposition's temporal network, either directly (primitive tasks) or indirectly (computation of min/max expression).
        \item Given the state trajectory associated to $\pi$ and the schedule, all constraints are valid.
    \end{itemize}

\end{definition}

\begin{definition}[Solution]
    A time-triggered plan $\pi$ is a solution to a Temporal HTN planning problem $P$ iff:
    \begin{itemize}
        \item $\pi$ is a solution to the non-hierarchical problem $P_{flat}$. In essence, it means that the plan is executable and yields a valid state trajectory that achieves all the goals.
        \item there exists a decomposition $w^*$ of $w_0$ such that $\pi$ is a valid schedule of $w^*$.
    \end{itemize}
...
Just a test to see how this goes, where $\states$ is the state trajectory induced by the plan.
\[
      \states \models(\holdduring \ t_1 \ t_2 \ \phi)
      \ \text{iff} \ \forall t \in [t_1,t_2], \ \states(t) \models \phi
\]
