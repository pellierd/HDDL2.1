...
\begin{itemize}
    ...
    \item ${\cal{C}}_v$ is a set of variable constraints. Each constraints can bind two task parameters to be equal or non-equals to an other one or a constant. % Peut être rajouter un exemple pour être symetrique
    
    \item ${\cal{C}}_d$ is a set of durative constraints over the duration of the tasks $\cal{I}$. For instance, let $t_1, t_2 \in I$ and $w$ a task network, the durative constraints $\duration(t_1) < \duration(t_2)$ expresses that the duration of $t_1$ must be strictly less that $t_2$, and the constraints $\duration(w) \geq \duration(t_1) * \duration(t_2)$ expresses that greater or equal to the product of duration of $t_1$ and $t_2$. Durative constraints can be expressed in terms of starting and ending of a task. For instance the previous constraint $\duration(t_1) < \duration(t_2)$ can be expressed as  $\tend(t_1) - \tstart(t_1) < \tend(t_2) - \tstart(t_2)$.
    
    \item ${\cal{C}}_t$ is a set of decomposition trajectory constraints of the form $(\at \ e \ \phi)$. This constraint expresses that some properties defined by the logical formula $\phi$ must hold in the state at the time event $e$. We will show in section~\ref{Sec:Decomposition-Constraints-Semantics}, how extends this simple constraint to more expressive ones proposed in HDDL~2.1.
   \end{itemize}
\end{definition}

Note that a temporal task network is ground if all its typed variables are bound to constant in $C$ and primitive if all its tasks ${\cal{I}}$ are primitive. It is now possible to formally defined a method. 
\begin{definition}[Method]
A {\em method} $m$ is a tuple $(\name(m), \task(m),$ $\tn(m))$, where $\name(m)$ is the name of the method, $\task(m)$ refined by the method and the $\tn(m))$ that defines the decomposition of the task $\task(m)$ into subtasks.
\end{definition}

\begin{definition}
 A {\em planning domain} $\cal{D}$ is a tuple $({\cal{L}}, {\cal{T}}, {\cal{A}}, {\cal{M}})$, where $\cal{L}$ is the underlying first order predicate logic, $\cal{T}$ is the set of tasks primitive and abstract, $\cal{A}$ a set of durative actions and $\cal{M}$ a set of methods. We note ${\cal{T}}_p$ the set of primitive tasks and ${\cal{T}}_a$ the set of abstract tasks, s.t.  ${\cal{T}} = {\cal{T}}_p \cup {\cal{T}}_a$.
 \end{definition}
 
 The domain implicitly defines the set of all states $S$, being defined over all subsets of all ground predicates in $\cal{L}$.
 
 \begin{definition}
 A {\em planning problem} $\cal{P}$ is a tuple $({\cal{D}}, {s_0}, w_0, g)$, where $\cal{D}$ is a planning domain, $s_0 \in S$ is the initial state, a ground conjunction of positive literals over the predicates of ${\cal{D}}$, $w_0$ the initial temporal task network not necessary ground and $g$ the goal description, a first order formula not necessary ground. 
 \end{definition}
 
 \section{Temporal HTN planning Semantic}
 \label{THTN}
